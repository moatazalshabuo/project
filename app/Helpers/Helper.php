...
class Helper
{
    ...
    {
        $mymaterial = DB::table('proudct_material')
            ->join("rawmaterials", "rawmaterials.id", "=", "proudct_material.rawid")
            ->select(DB::raw("SUM((rawmaterials.pace_price * proudct_material.quan)) as sum"))
    ...
    public static function minus_from_mate($id)
    {
        ...
        foreach ($mate as $val) {
            ...
            if ($raw->material_type == 3)
            {
                // die(ceil($val->quan * $salesItem->qoun));
                $raw->quantity = ($raw->quantity - ($val->quan * ceil($salesItem->qoun)));
            }else{
    ...
    public static function add_from_mate($id)
    {
        ...
        foreach ($mate as $val) {
            $raw = rawmaterials::find($val->rawid);
            if ($raw->material_type == 3)
                $raw->quantity = ($raw->quantity + ceil($val->quan * $salesItem->qoun));
            else
                $raw->quantity = ($raw->quantity + $val->coun);
            $raw->update();
        }
