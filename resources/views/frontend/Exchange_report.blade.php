...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">تقرير</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ ايصال
                    الصرف</span>
            </div>
        </div>
                ...
                <div class="card-body">
                    <form action="{{ route('search_exc') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <label>العميل</label>
                                <select name="custom" class="form-control sel">
                                ...
                                @if ($data)
                                    @foreach ($data as $item)
                                        <tr>
                                            <td>{{ $item->id }}</td>
                                            @if ($item->bill_id)
                                                <td><a href="{{ route('Purchasesbill', $item->bill_id) }}"> فاتورة مشتريات
                                                        رقم{{ $item->bill_id }}</a></td>
                                            @else
                                                <td>{{ $item->desc }}</td>
                                            @endif
