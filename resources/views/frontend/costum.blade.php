...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">كشف حساب مورد </h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"></span>
						</div>
					</div>
					...
					<div class="col-md-12">
						<div class="card">
											...
											@if ( session()->get('data'))
												@foreach (session()->get('data') as $item)
													<tr>
														<td>
															@if ( $item['type_n'] == '2')
															<a href="{{ route('Purchasesbill',$item['id_bill']) }}"> فاتورة المشتريات رقم{{ $item['id_bill'] }} </a>
															@else
							...
							<div class="card-footer">
								<button class="btn btn-pink-gradient">خالص : <span>@if (session()->get('sincere_s'))
									{{ session()->get('sincere_s') }}
								@endif</span></button>
								<button class="btn btn-pink-gradient">متبقي له : <span>@if (session()->get('Residual_s'))
