...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">تقرير</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ فواتير المشتريات</span>
						</div>
					</div>
										...
										<tbody class="" id="myTable">
											@if ( session()->get('data'))
												@foreach (session()->get('data') as $item)
													<tr>
														<td><a href="{{ route('salesbill',$item->id) }}"> فاتورة مشتريات رقم{{ $item->id }} </a></td>
														{{-- <td><a href="{{ route("Purchasesbill",$item->bill_id) }}"> فاتورة مشتريات رقم{{ $item->bill_id }}</a></td> --}}
														<td>{{ floatval($item->tolal) }}</td>
