...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">اعمال لم تستلم</h4><span
                        ...
                        <table class="table text-md-nowrap text-center" id="">
                            ...
                            <tbody class="" id="myTable">
                                @foreach ($data as $item)
                                    <tr>
                                        <td><input name="done" type="checkbox" value="{{ $item->id }}"
                                                class="disabled"></td>
                                        <td>{{ $item->name }}</td>
                                        <td>{{ $item->descripe }}</td>
                                        <td><a href="{{ route('salesbill', $item->sales_id) }}">فاتورة رقم
                                                {{ $item->sales_id }}</a></td>
                                        @if ($item->type_Q == 3)
                                        ...
                                        <td class="d-flex">
                                            <a href="{{ route('its_completed', $item->id) }}?back=true"
                                                class="btn btn-outline-danger">تراجع</a>
                                            <a href="{{ route('its_completed', $item->id) }}"
                                                class="btn btn-outline-success">استلام</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
